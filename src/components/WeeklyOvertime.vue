...
const fetchWorkTimeData = async () => {
  try {
    ...
    if (weekOptions.value.length > 0) {
      selectedWeek.value =
        weekOptions.value[weekOptions.value.length - 1].weekID;
      updateChart();
    } else {
      hasData.value = false;
...
const updateChart = async () => {
  ...
  const selectedData = workTimeData.value.filter(
    (entry) =>
      validStatuses.includes(entry.status) &&
      entry.weekID === selectedWeek.value
  );
  const selectedWeekInfo = weekOptions.value.find((w) =>
    typeof w === "object"
      ? w.weekID === selectedWeek.value
  ...
  const processes = Object.keys(processMap).sort(
    (a, b) => processMap[b] - processMap[a]
  );
  ...
  const layout = {
    ...
    title: `Weekly Overtime by Process — Week ${selectedWeek.value} ${dateRange}`,
    width: chartWidth,
    xaxis: {
      title: "Process",
      tickangle: -45,
