      ...
      <table class="widgets-table">
        ...
        <tbody>
          <tr v-if="widgets.length === 0">
            <td colspan="8" class="no-data">No widgets found</td>
          </tr>
          <tr v-for="widget in widgets" :key="widget.widgetId">
            <!-- <td><code>{{ widget.widgetId }}</code></td> -->
            <td>
              <strong>{{ widget.displayName }}</strong>
              <small v-if="widget.description" class="description">
                {{ widget.description }}
              </small>
            </td>
            <td>
      ...
      <div class="modal modal-small">
        ...
        <div class="modal-body">
          ...
          <div class="delete-info">
            <strong>{{ widgetToDelete?.displayName }}</strong>
            <code>{{ widgetToDelete?.widgetId }}</code>
          </div>
          <p class="warning-text">
            ⚠️ This action cannot be undone. All user configurations for this widget will be affected.
          </p>
        </div>
...
const fetchWidgets = async () => {
  try {
    ...
    errorMessage.value = `Failed to load widgets: ${error.response?.data || error.message}`;
  } finally {
    loading.value = false;
  }
};
...
const openAddModal = () => {
  ...
  formData.value = {
    ...
    span2: false,
    isActive: true,
    displayOrder: widgets.value.length,
    description: '',
  };
...
const openEditModal = (widget) => {
  ...
  // ✅ แปลง property names ให้ตรงกับ form
  formData.value = {
    widgetId: widget.widgetId,
    displayName: widget.displayName,
    componentName: widget.componentName,
    componentPath: widget.componentPath || '',
    span2: widget.span2 || false,
    isActive: widget.isActive !== false, // default true if undefined
    displayOrder: widget.displayOrder || 0,
    description: widget.description || ''
  };
  
  console.log('📝 Editing widget:', formData.value);
  showModal.value = true;
};
...
const saveWidget = async () => {
  try {
    // Validation
    if (!formData.value.widgetId || !formData.value.displayName || !formData.value.componentName) {
      errorMessage.value = 'Please fill in all required fields (ID, Name, Component)';
      return;
    }
    ...
    if (isEditMode.value) {
      // Update
      await axios.put(`${API_BASE}/AdminWidget/${formData.value.widgetId}`, formData.value);
      successMessage.value = `Widget "${formData.value.displayName}" updated successfully`;
    } else {
      // Create
      await axios.post(`${API_BASE}/AdminWidget`, formData.value);
    ...
    errorMessage.value = `Failed to save widget: ${error.response?.data || error.message}`;
  } finally {
    loading.value = false;
  }
};
...
const toggleWidgetStatus = async (widget) => {
  try {
    ...
    await axios.patch(`${API_BASE}/AdminWidget/${widget.widgetId}/toggle`);
    
    successMessage.value = `Widget "${widget.displayName}" ${!widget.isActive ? 'activated' : 'deactivated'}`;
    ...
    errorMessage.value = `Failed to update widget status: ${error.response?.data || error.message}`;
  } finally {
    loading.value = false;
  }
};
...
const confirmDelete = async () => {
  try {
    ...
    await axios.delete(`${API_BASE}/AdminWidget/${widgetToDelete.value.widgetId}`);
    ...
    errorMessage.value = `Failed to delete widget: ${error.response?.data || error.message}`;
  } finally {
    loading.value = false;
  }
};
